...
export function Watchlist() {
  ...
  const filtered = useMemo(
    () =>
      activeTag
        ? items.filter(
            (w) => w.tags.includes(activeTag) || (w.autoTags ?? []).includes(activeTag)
          )
        : items,
    [items, activeTag]
  );
  ...
  const sortedTags = useMemo(() => {
    ...
    const result: string[] = [];
    if (set.has(PORTFOLIO_AUTO_TAG)) {
      result.push(PORTFOLIO_AUTO_TAG);
      set.delete(PORTFOLIO_AUTO_TAG);
    }
    if (set.has(DIVIDEND_AUTO_TAG)) {
      result.push(DIVIDEND_AUTO_TAG);
      set.delete(DIVIDEND_AUTO_TAG);
    }
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  tag === activeTag
                    ? 'bg-indigo-600 text-white'
                    : tag === PORTFOLIO_AUTO_TAG
                      ? 'bg-emerald-100 dark:bg-emerald-900/30 text-emerald-700 dark:text-emerald-400 hover:bg-emerald-200 dark:hover:bg-emerald-900/50'
                      : tag === DIVIDEND_AUTO_TAG
...
function WatchlistRow({
  ...
  /** Display order: auto-tags first (portfolio, dividend, recommendation), then user tags alphabetically. */
  const allChips = useMemo(() => {
    const autos = [...item.autoTags].sort((a, b) => a.localeCompare(b));
    const userTags = [...item.tags].sort((a, b) => a.localeCompare(b));
    return [...autos, ...userTags];
  }, [item.autoTags, item.tags]);
  ...
  function handleTagKeyDown(e: React.KeyboardEvent) {
    if (e.key === 'Enter' || e.key === ',') {
      ...
      if (isAutoGeneratedTag(tag) || item.tags.includes(tag)) {
        setTagInput('');
        return;
      }
  ...
  function removeTag(tag: string) {
    if (isAutoGeneratedTag(tag) || !item.id) return;
    updateWatchlistItem(item.id, {
    ...
    <div className="border-b border-gray-50 dark:border-slate-800/50 last:border-b-0">
      ...
      {showTagEditor && (
        <div className="px-4 pb-3 flex items-center gap-2 flex-wrap" onClick={stopProp}>
          {allChips.map((tag) => {
            const isAuto = isAutoGeneratedTag(tag);
            return (
              <span
                key={tag}
                className={cn(
